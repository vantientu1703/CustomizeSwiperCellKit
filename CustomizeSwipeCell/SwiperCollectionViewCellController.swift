//
//  SwiperCollectionViewCellController.swift
//  CustomizeSwipeCell
//
//  Created by Van Tien Tu on 4/17/20.
...
protocol SwiperCollectionViewCellControllerDelegate: class {
...
class SwiperCollectionViewCellController: NSObject {
    
    var swipeable: Swipeable?
    weak var actionsContainerView: UIView?
    
    weak var delegate: SwiperCollectionViewCellControllerDelegate?
    ...
    init(swipeable: Swipeable, actionsContainerView: UIView) {
    ...
    @objc func handlePan(gesture: UIPanGestureRecognizer) {
        ...
        // new
        guard let cell = (swipeable as? MockerSwipeController)?.cell else { return }
        ...
        case .began:
            ...
            */
            ...
            if let swipeable = scrollView?.swipeables.first(where: { $0.state == .dragging }) as? UIView, self.swipeable != nil, swipeable != cell {
                return
            }
        ...
        case .changed:
            ...
            if swipeable.state == .animatingToCenter {
                ...
                if let swipedCell = swipedCell, self.swipeable != nil, swipedCell != cell {
                    return
                }
            }
            ...
            if (translation + originalCenter - cell.bounds.midX) * actionsView.orientation.scale > 0 {
            ...
            if let expansionStyle = actionsView.options.expansionStyle, let scrollView = scrollView {
                ...
                let referenceFrame = actionsContainerView != cell ? actionsContainerView.frame : nil;
                ...
                let currentOffset = abs(translation + originalCenter - cell.bounds.midX)
                
                if expanded && !actionsView.expanded && targetOffset > currentOffset {
                    let centerForTranslationToEdge = cell.bounds.midX - targetOffset * actionsView.orientation.scale
                    let delta = centerForTranslationToEdge - originalCenter
                    
                    animate(toOffset: centerForTranslationToEdge)
                    gesture.setTranslation(CGPoint(x: delta, y: 0), in: cell.superview!)
                } else {
                    target.center.x = gesture.elasticTranslation(in: target,
        ...
        case .ended, .cancelled, .failed:
            guard let actionsView = swipeable.actionsView, let actionsContainerView = self.actionsContainerView else { return }
            if swipeable.state.isActive == false && cell.bounds.midX == target.center.x  {
                return
            }
    ...
    func showActionsView(for orientation: SwipeActionsOrientation) -> Bool {
        ...
        guard let cell = (swipeable as? MockerSwipeController)?.cell else { return false }
        originalLayoutMargins = cell.layoutMargins
    ...
    func configureActionsView(with actions: [SwipeAction], for orientation: SwipeActionsOrientation) {
        ...
        actionsView.topAnchor.constraint(equalTo: cell.topAnchor).isActive = true
    ...
    func animate(duration: Double = 0.7, toOffset offset: CGFloat, withInitialVelocity velocity: CGFloat = 0, completion: ((Bool) -> Void)? = nil) {
        stopAnimatorIfNeeded()
        let cell = (self.swipeable as? MockerSwipeController)?.cell
        cell?.layoutIfNeeded()
    ...
    func targetCenter(active: Bool) -> CGFloat {
        ...
        guard let cell = (swipeable as? MockerSwipeController)?.cell else { return 0 }
        guard let actionsView = swipeable.actionsView, active == true else { return cell.bounds.midX }
        return cell.bounds.midX - actionsView.preferredWidth * actionsView.orientation.scale
    }
...
extension SwiperCollectionViewCellController: UIGestureRecognizerDelegate {
...
extension SwiperCollectionViewCellController: SwipeActionsViewDelegate {
    ...
    func performFillAction(action: SwipeAction, fillOption: SwipeExpansionStyle.FillOptions) {
        ...
        guard let cell = (swipeable as? MockerSwipeController)?.cell else { return }
        let newCenter = cell.bounds.midX - (cell.bounds.width + actionsView.minimumButtonWidth) * actionsView.orientation.scale
    ...
    func hideSwipe(animated: Bool, completion: ((Bool) -> Void)? = nil) {
        ...
        //guard let s = swipeable as? MockSwipeController else { return }
    ...
    func setSwipeOffset(_ offset: CGFloat, animated: Bool = true, completion: ((Bool) -> Void)? = nil) {
        ...
        let maxOffset = min(cell.bounds.width, abs(offset)) * orientation.scale * -1
        let targetCenter = abs(offset) == CGFloat.greatestFiniteMagnitude ? self.targetCenter(active: true) : cell.bounds.midX + maxOffset
